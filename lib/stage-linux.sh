...
if (( iso_bytes <= fat32_limit )); then
  carrier_fs=fat32; carrier_label=LINUXINST
else
  carrier_fs=exfat; carrier_label=LINUXINST
fi
...
# Inspect the ISO to choose kernel/initrd and boot parameters.
ISO_MNT=$(mktemp -d)
...
KERNEL=""; INITRD=""; FAMILY=generic
find_first() { local p; for p in "$@"; do [[ -f "$ISO_MNT/$p" ]] && { printf '/%s' "$p"; return; }; done; }
if [[ -d $ISO_MNT/casper ]]; then
  FAMILY=casper
  KERNEL=$(find_first casper/vmlinuz casper/vmlinuz.efi)
  INITRD=$(find_first casper/initrd casper/initrd.lz casper/initrd.img)
elif [[ -d $ISO_MNT/live ]]; then
  FAMILY=live
  KERNEL=$(find_first live/vmlinuz live/vmlinuz.efi)
  INITRD=$(find_first live/initrd.img live/initrd)
elif [[ -d $ISO_MNT/arch ]]; then
  FAMILY=arch
  KERNEL=$(find_first arch/boot/x86_64/vmlinuz-linux arch/boot/x86_64/vmlinuz)
  INITRD=$(find_first arch/boot/x86_64/initramfs-linux.img)
elif [[ -d $ISO_MNT/images/pxeboot ]]; then
  FAMILY=fedora
  KERNEL=$(find_first images/pxeboot/vmlinuz)
  INITRD=$(find_first images/pxeboot/initrd.img)
else
  KERNEL=$(find_first install.amd/vmlinuz install/vmlinuz boot/vmlinuz)
  INITRD=$(find_first install.amd/initrd.gz install/initrd.gz boot/initrd.img)
fi
cleanup; trap - EXIT

[[ -n $KERNEL && -n $INITRD ]] || die "could not locate a kernel/initrd in the ISO (distro unsupported in this beta)"
log "  Detected: family=$FAMILY kernel=$KERNEL initrd=$INITRD"

# Kernel arguments per family.
case "$FAMILY" in
...
cp --reflink=auto "$ISO" "$CARRIER_MNT/$iso_name"
sync
...
log "Note: with Secure Boot on, this GRUB is unsigned; disable Secure Boot or sign it first."
